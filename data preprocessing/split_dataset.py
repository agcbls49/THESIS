...
def create_output_dirs():
    # Create the images and labels folders for each split inside the output directory
    for split in SPLITS:
...
def get_valid_pairs(class_folder_path):
    ...
    # Collect all image and label files, using their name (without extension) as the key
    ...
    # Check that every image has a matching label, and vice versa, raise an error if any are missing
    for stem in images:
        if stem in labels:
...
def split_pairs(pairs):
    # Shuffle the pairs randomly using the seed for reproducibility
    random.seed(SEED)
    shuffled = pairs[:]
    random.shuffle(shuffled)

    # Calculate the cutoff indices for train and val, the remainder goes to test
    total = len(shuffled)
...
def copy_files(pairs, split, class_folder_path):
    # Copy each image and label file to their corresponding split folder in the output directory
...
def main():
    # Raise an error if the source directory doesn't exist
    if not os.path.isdir(SOURCE_DIR):
        raise NotADirectoryError(f"Source directory '{SOURCE_DIR}' not found.")

    create_output_dirs()

    # Get all subfolders in the source directory, each one is treated as a class
    class_folders = [
    ...
    for class_name in sorted(class_folders):
        ...
        # Copy files for each split and keep a running count for the summary
    ...
    # Print the final summary of how many pairs ended up in each split
    print("\nDone!")
